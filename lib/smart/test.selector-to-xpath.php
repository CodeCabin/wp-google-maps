	...
	public function peek($expectedType=null, $calledByRead=0)
	{
		...
		if($expectedType !== null)
		{
			...
			if($token->type != $expectedType)
				throw new ParseException('Unexpected ' . $token->getFriendlyType() . ' "' . $token->string . '"');
		}
...
class XPathConverter
{
	...
	public function convert($str)
	{
		...
		var_dump($this->xpath);
		trace(implode('', $this->xpath));
	}
}

$toXPath = new XPathConverter();
$result = $toXPath->convert('div.a.b#page:nth-child( -3 + 6n ),[ contenteditable ]:last-child, 
	elem[attribute="string with \" double escape"],
	test\.node,
	div > span > figure#test, 
	E[ foo~="bar" ]    title,
	blockquote~footer,
	input[name="idusers[]"],
	*[name="price"],
...
/*$result = $test->parse('[ contenteditable ]:last-child, 
	elem[attribute="string with \\\\\" double escape"],
	test\.node,
	div > span > figure#test, 
	E[ foo~="bar" ]    title,
